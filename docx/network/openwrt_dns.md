...
2.  导航到 **网络 (Network)** -> **接口 (Interfaces)**。
3.  找到并编辑您的 **WAN** 接口（通常是连接到互联网的接口）。
4.  在 **高级设置 (Advanced Settings)** 选项卡下，取消勾选 **“使用路由器提供的 DNS 服务器 (Use DNS servers advertised by peer)”**。
5.  在下方出现的 **“使用自定义 DNS 服务器 (Use custom DNS servers)”** 框中，输入您想要使用的 DNS 服务器 IP 地址，例如 Cloudflare 的 `1.1.1.1` 或 Google 的 `8.8.8.8`。您可以添加多个以空格分隔。
6.  点击 **保存 (Save)** 并 **应用 (Apply)**。
...
3.  在 **DHCP** 选项卡下的 **“高级设置 (Advanced Settings)”** 中，找到 **“DHCP 选项 (DHCP-Options)”** 字段。
4.  在其中输入 `6,DNS1,DNS2` 的格式。例如，如果您想使用 Cloudflare 的 DNS，可以输入 `6,1.1.1.1,1.0.0.1`。
5.  点击 **保存 (Save)** 并 **应用 (Apply)**。
